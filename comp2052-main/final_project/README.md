# 📊 Sistema de Encuestas y Votaciones (Proyecto Demo) - Flask + MySQL

Este proyecto permite gestionar encuestas donde **Moderadores** o **Administradores** pueden crear y administrar encuestas, y los **Votantes** pueden participar en ellas. Los **Administradores** también pueden gestionar usuarios y roles. Este es una adaptación del Proyecto 9 de la colección de proyectos finales.
...
| `app/models.py`                                                   | Contiene los modelos SQLAlchemy: User, Role, Encuesta                      |
| `app/forms.py`                                                    | Formularios de Flask-WTF usados en login, registro, encuestas, contraseñas |
| `app/routes.py`                                                   | Rutas principales del proyecto (dashboard, encuestas, cambiar contraseña)  |
| `app/auth_routes.py`                                              | Rutas para autenticación (login, registro, logout)                         |
| `app/test_routes.py`                                              | Rutas (end-points) para pruebas relacionadas con encuestas (/encuestas). Asegúrate de actualizar las consultas y las plantillas usadas para realizar las pruebas de tu proyecto.                                          |
...
| `app/templates/encuesta_form.html`                                | Formulario de creación/edición de encuestas                                |
| `app/templates/dashboard.html` (anteriormente `cursos.html`)      | Muestra la lista de encuestas (funcionalidad de `cursos.html` integrada aquí) |
...
| `database_schema/09_encuestas.sql`                                | SQL para crear la base de datos y tablas del proyecto de encuestas         |
...
4. **Crear la base de datos en MySQL**
   ...
   ```bash
   mysql -u root -p < database_schema/09_encuestas.sql
   ```
...
| Moderador  | Mod Example   | moderador@example.com  | moderador123 |
| Votante    | Voter Example | votante@example.com    | votante123   |
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app/models.py`                      | Renombrar el modelo Curso al nuevo recurso principal (Encuesta). Cambiar atributos de la entidad principal según el nuevo CRUD.                                                                                             |
| `app/forms.py`                       | Modificar CursoForm (ahora EncuestaForm) para reflejar los datos o campos de tu entidad principal (Encuesta). Especifica el tipo de control de entrada según corresponda al dato de tu entidad principal.                |
| `app/routes.py`                      | Cambiar las rutas relacionadas con cursos (e.g. /cursos a /encuestas, /editar_curso a /editar_encuesta, etc.) al nuevo recurso. Asegúrate de actualizar las consultas y las plantillas usadas.                           |
| `app/test_routes.py`                 | Rutas (end-points) para pruebas relacionadas con encuestas (/encuestas). Asegúrate de actualizar las consultas y las plantillas usadas para realizar las pruebas de tu proyecto.                                             |
| `templates/encuesta_form.html`       | Renombrar el archivo de `curso_form.html` y cambiar los datos (cajas de texto, etc.) o campos que se muestran en el formulario para encuestas.                                                                           |
| `templates/dashboard.html`           | (Anteriormente `cursos.html` para listar) Actualiza la tabla en `dashboard.html` para mostrar los datos específicos de las encuestas.                                                                                     |
| `database_schema/09_encuestas.sql`   | Verifica que el archivo `09_encuestas.sql` esté actualizado según los datos que solicitarás para tu proyecto (Encuesta, User, Role). Puedes editarlo o usarlo como guía.                                                      |
